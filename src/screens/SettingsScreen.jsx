...
function Interrupteur({ actif, onChange }) {
  return (
    ...
    >
      <span className="absolute top-0.5 w-4 h-4 rounded-full bg-white transition-all duration-200" style={{ left: actif ? '20px' : '3px' }} />
    </button>
  )
}
...
function SettingsScreen({ onChangerEcran }) {
  ...
  const [couleursEnEdition, setCouleursEnEdition] = useState(() => {
    const base = parametres.couleursPersonnalisees || THEMES_DISPONIBLES.find((t) => t.id === parametres.themeId).couleurs
    return { ...base }
  })
  ...
  const exporterMesDonnees = () => {
    const donnees = {}
    Object.keys(localStorage).filter((cle) => cle.startsWith('yuna-')).forEach((cle) => {
      try { donnees[cle] = JSON.parse(localStorage.getItem(cle)) } catch { donnees[cle] = localStorage.getItem(cle) }
    })
  ...
  const reinitialiserToutesLesDonnees = () => {
    const confirme = window.confirm('Ceci supprimera définitivement tes conversations, ta galerie, ton profil et tes paramètres. Cette action est irréversible. Continuer ?')
    if (!confirme) return
    Object.keys(localStorage).filter((cle) => cle.startsWith('yuna-')).forEach((cle) => localStorage.removeItem(cle))
    ...
    <div className="h-full min-h-0 w-full flex flex-col overflow-hidden bg-[#F0EEEB]">
      ...
      <div className="flex items-center gap-3 px-4 md:px-7 py-4 md:py-6 flex-shrink-0">
        ...
        <div className="min-w-0">
          <h1 className="text-espresso font-semibold truncate" style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '19px' }}>
            Paramètres
          </h1>
          <p className="text-[10px] md:text-[10.5px] text-espresso/45">Personnalise Yuna à ton image</p>
        </div>
        ...
        <div className="px-4 md:px-7 pb-7 md:overflow-y-auto md:scroll-suave md:min-h-0">
          ...
          <SectionParametre titre="Yuna & moi" description="Comment Yuna doit s'adresser à toi">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label className="text-[9px] text-espresso/40 uppercase tracking-wide">Comment Yuna doit t'appeler</label>
                <input type="text" value={parametres.surnom} onChange={(e) => mettreAJourParametres({ surnom: e.target.value })}
              ...
              <div>
                <label className="text-[9px] text-espresso/40 uppercase tracking-wide">Date d'anniversaire</label>
                <input type="date" value={parametres.dateAnniversaire} onChange={(e) => mettreAJourParametres({ dateAnniversaire: e.target.value })}
                  className="w-full bg-[#F0EEEB] rounded-xl px-3 py-2 text-[12px] text-espresso mt-1 outline-none border border-espresso/15 focus:border-espresso transition-colors duration-200" />
              </div>
            </div>
          </SectionParametre>
            ...
            <div className={parametres.messagesActifs ? '' : 'opacity-40 pointer-events-none'}>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="time" value={parametres.heureDebut} onChange={(e) => mettreAJourParametres({ heureDebut: e.target.value })}
                ...
                <div>
                  ...
                  <input type="time" value={parametres.heureFin} onChange={(e) => mettreAJourParametres({ heureFin: e.target.value })}
                    className="w-full bg-[#F0EEEB] rounded-xl px-3 py-2 text-[12px] text-espresso mt-1 outline-none border border-espresso/15 focus:border-espresso" />
                </div>
              </div>
            </div>
        ...
        >
          ...
          <SectionParametre titre="Notifications & accessibilité">
            <div className="flex items-center justify-between mb-3">
              ...
              <Interrupteur actif={parametres.notificationsActives} onChange={(val) => mettreAJourParametres({ notificationsActives: val })} />
            </div>
          ...
          <SectionParametre titre="Données & confidentialité" description="Tes données restent dans ce navigateur, jamais envoyées ailleurs">
            <div className="flex flex-col gap-2.5">
              <button onClick={exporterMesDonnees}
              ...
              <button onClick={reinitialiserToutesLesDonnees}
          ...
          <div className="md:hidden bg-white rounded-2xl p-4 border border-espresso/10">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h2 className="text-espresso/45 text-[10.5px] uppercase tracking-[0.08em] font-medium">À propos</h2>
            </div>
          ...
          <div className={themeOuvert ? 'block' : 'hidden md:block'}>
            <div className="flex flex-col gap-2 mb-4">
              {THEMES_DISPONIBLES.map((theme) => {
                ...
                return (
                  <button key={theme.id} onClick={() => { choisirTheme(theme.id); setModePerso(false); afficherConfirmation() }}
                    ...
                    <div className="flex -space-x-1.5 flex-shrink-0">
                      {['espresso', 'peony', 'accent'].map((cle) => (
                        <span key={cle} className="w-5 h-5 rounded-full border-2 border-white" style={{ backgroundColor: theme.couleurs[cle] }} />
                      ))}
                    </div>
            ...
            <button onClick={() => setModePerso(!modePerso)}
                ...
                <div className="flex flex-col gap-3 mb-3">
                  {[
                    ...
                    <div key={champ.cle} className="flex items-center justify-between gap-2">
                      ...
                      <input type="color" value={couleursEnEdition[champ.cle]}
                        onChange={(e) => setCouleursEnEdition((a) => ({ ...a, [champ.cle]: e.target.value }))}
                        className="w-14 h-8 rounded-lg cursor-pointer border border-espresso/15" />
                    </div>
                  ))}
                </div>
                <button onClick={validerCouleursPersonnalisees}
