  ...
  return (
    <button
      ...
      className={`relative rounded-full transition-colors duration-200 flex-shrink-0 ${
        actif ? 'bg-espresso' : 'bg-espresso/15'
      }`}
...
// ============================================================
// ENCADRÉ DE SECTION réutilisable
// ============================================================
function SectionParametre({ titre, description, children }) {
  return (
    <div className="bg-white rounded-2xl p-5 border border-espresso/10 mb-4">
      <h3 className="text-[13px] font-semibold text-espresso mb-0.5">{titre}</h3>
      {description && <p className="text-[10.5px] text-espresso/45 mb-4">{description}</p>}
...
function SettingsScreen({ onChangerEcran }) {
  ...
  const [couleursEnEdition, setCouleursEnEdition] = useState(() => {
    const base = parametres.couleursPersonnalisees
  ...
  return (
    // ============================================================
    // RETOUR AU PLEIN CADRE (edge-to-edge)
    // Même logique que HomeScreen / ChatScreen / ProfileScreen :
    // h-full + w-full, pas de marge ni de fond coloré autour,
    // pour que ça s'intègre exactement pareil dans le layout de l'app.
    // ============================================================
    <div className="h-full w-full flex flex-col overflow-hidden bg-[#F0EEEB]">

      {/* ===== EN-TÊTE (pleine largeur, fixe) ===== */}
      <div className="flex items-center gap-3 px-7 py-6 flex-shrink-0">
        <div className="w-10 h-10 rounded-full flex items-center justify-center bg-espresso/8">
        ...
        <div>
          ...
          <p className="text-[10.5px] text-espresso/45">Personnalise Yuna à ton image</p>
        </div>

        {messageConfirmation && (
          <span className="ml-auto flex items-center gap-1.5 text-[11px] font-medium text-emerald-600 bg-emerald-50 px-3 py-1.5 rounded-full">
            <IconCoche style={{ width: '12px', height: '12px' }} />
            Enregistré
          </span>
        )}
      </div>

      {/* ===== CORPS : colonne principale + colonne thème ===== */}
      <div className="flex-1 grid grid-cols-[1fr_300px] overflow-hidden">

        {/* ----- COLONNE GAUCHE : réglages fonctionnels ----- */}
        <div className="px-7 pb-7 overflow-y-auto">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => mettreAJourParametres({ surnom: e.target.value })}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => mettreAJourParametres({ dateAnniversaire: e.target.value })}
                  className="w-full bg-[#F0EEEB] rounded-xl px-3 py-2 text-[12px] text-espresso mt-1 outline-none border border-espresso/15 focus:border-espresso transition-colors duration-200"
                />
              </div>
            </div>
            {parametres.dateAnniversaire && (
              <p className="text-[10px] text-espresso/40 italic mt-2">
                🎂 Yuna t'enverra un message spécial ce jour-là
              </p>
            )}
          </SectionParametre>
          ...
          <SectionParametre titre="Messages spontanés">
            <div className="flex items-center justify-between mb-4">
              <div>
                <p className="text-[11.5px] text-espresso font-medium">Autoriser Yuna à t'écrire seule</p>
                <p className="text-[10px] text-espresso/45">Elle pourra t'envoyer des messages sans que tu lui parles d'abord</p>
              </div>
              <Interrupteur
                actif={parametres.messagesActifs}
                onChange={(val) => mettreAJourParametres({ messagesActifs: val })}
              />
            </div>
            ...
            <div className={parametres.messagesActifs ? '' : 'opacity-40 pointer-events-none'}>
              ...
              <select
                ...
                onChange={(e) => mettreAJourParametres({ frequence: e.target.value })}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => mettreAJourParametres({ heureDebut: e.target.value })}
                ...
                <div>
                  <label className="text-[9px] text-espresso/40 uppercase tracking-wide">Pas après</label>
                  <input
                    type="time"
                    value={parametres.heureFin}
                    onChange={(e) => mettreAJourParametres({ heureFin: e.target.value })}
                    className="w-full bg-[#F0EEEB] rounded-xl px-3 py-2 text-[12px] text-espresso mt-1 outline-none border border-espresso/15 focus:border-espresso"
                  />
                </div>
              </div>
                  ...
                  <button
                    ...
                    className={`text-left rounded-xl p-3 border transition-all duration-200 ${
                      estActif
                        ...
                        : 'bg-[#F0EEEB] border-espresso/12 hover:border-espresso/30'
                    }`}
                  >
                    <p className={`text-[12px] font-semibold ${estActif ? 'text-peony' : 'text-espresso'}`}>
                      {p.label}
                    </p>
                    <p className={`text-[9.5px] mt-0.5 ${estActif ? 'text-peony/70' : 'text-espresso/45'}`}>
                      {p.desc}
                    </p>
                  </button>
                )
          ...
          <SectionParametre titre="Notifications & accessibilité">
            <div className="flex items-center justify-between mb-3">
              ...
              <Interrupteur
                ...
                onChange={(val) => mettreAJourParametres({ notificationsActives: val })}
              />
            </div>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-[11.5px] text-espresso font-medium">Réduire les animations</p>
                <p className="text-[10px] text-espresso/45">Coupe les transitions et effets de mouvement</p>
              </div>
              <Interrupteur
                actif={parametres.reduireAnimations}
                onChange={(val) => mettreAJourParametres({ reduireAnimations: val })}
              />
            </div>
          </SectionParametre>
            ...
            <div className="flex flex-col gap-2.5">
              ...
              >
                <div className="flex items-center gap-2.5">
                  <IconDownload style={{ width: '15px', height: '15px' }} className="text-espresso/60" />
                  <span className="text-[11.5px] text-espresso font-medium">Exporter mes données</span>
                </div>
              ...
              >
                <IconTrash style={{ width: '15px', height: '15px' }} className="text-red-500" />
                <span className="text-[11.5px] text-red-600 font-medium">Tout réinitialiser</span>
              </button>
            </div>
          </SectionParametre>
        </div>

        {/* ----- COLONNE DROITE : thème visuel + à propos (fond blanc) ----- */}
        <div className="px-6 py-7 bg-white border-l border-espresso/10 overflow-y-auto">
          ...
          <div className="flex flex-col gap-2 mb-4">
            {THEMES_DISPONIBLES.map((theme) => {
              const estActif = parametres.themeId === theme.id && !parametres.couleursPersonnalisees
              return (
                <button
                  key={theme.id}
                  onClick={() => { choisirTheme(theme.id); setModePerso(false); afficherConfirmation() }}
                  className={`flex items-center gap-2.5 rounded-xl p-3 border transition-all duration-200 ${
                    estActif ? 'border-espresso bg-[#F0EEEB]' : 'border-espresso/12 hover:border-espresso/30'
                  }`}
                >
                  ...
                  <span className="text-[11px] text-espresso font-medium">{theme.nom}</span>
                  {estActif && <IconCoche style={{ width: '12px', height: '12px' }} className="text-espresso ml-auto" />}
                </button>
              )
            })}
          </div>
          ...
          >
            ...
            {modePerso ? 'Masquer la personnalisation' : 'Créer mes propres couleurs'}
          </button>

          {modePerso && (
            <div className="pt-3 border-t border-espresso/10 mb-6">
              <div className="flex flex-col gap-3 mb-3">
                {[
                  ...
                  <div key={champ.cle} className="flex items-center justify-between gap-2">
                    <label className="text-[10px] text-espresso/50">{champ.label}</label>
                    <input
                      type="color"
                      value={couleursEnEdition[champ.cle]}
                      onChange={(e) =>
                        setCouleursEnEdition((ancien) => ({ ...ancien, [champ.cle]: e.target.value }))
                      }
          ...
          <div className="pt-4 border-t border-espresso/10">
            ...
            <p className="text-[11px] text-espresso font-semibold mb-0.5">Yuna</p>
            <p className="text-[10px] text-espresso/45 mb-3">Version {VERSION_APP}</p>
            <p className="text-[10px] text-espresso/50 leading-relaxed">
              Une IA compagnon créée avec React, Tailwind CSS et l'API Gemini — projet personnel développé en L2.
            </p>
          </div>
        </div>
      </div>
