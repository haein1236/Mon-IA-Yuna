...
// Sans ça, si une API reste "en attente" sans jamais répondre ni
// échouer, l'interface (les petits points "...") restait bloquée
// pour toujours, sans aucune erreur visible dans la console.
// Promise.race() fait "gagner" quel que soit le premier terminé :
// soit la vraie réponse arrive à temps, soit le timeout se déclenche
// et transforme l'attente infinie en une vraie erreur exploitable.
// ============================================================
...
const DESCRIPTIONS_PERSONNALITE = {
  ...
  romantique:
    "Tu aimes parler d'amour, de rendez-vous et d'émotions avec douceur et poésie — comme une amie qui adore romancer la vie de tous les jours. Tu discutes DE ces sujets avec la personne, tu ne te positionnes jamais toi-même comme sa partenaire romantique.",
...
const EXEMPLES_PERSONNALITE = {
  ...
  girlbestie: [
    { user: "Je suis crevée aujourd'hui...", yuna: "Oh nooon raconte tout 👀" },
  ],
  fashion: [
    {
      user: "Je suis crevée aujourd'hui...",
  ...
  try {
    ...
    const historiqueFormate = historique.map((msg) => ({
      ...
      parts: [
        {
          text:
            msg.texte === "[NOTE_VOCALE]" ? "[note vocale envoyée]" : msg.texte,
        },
      ],
    }));
...
async function genererResumePersonnage(ancienResume, messagesACondenser) {
  try {
    ...
    const texteAResumer = messagesACondenser
      .map(
        (msg) =>
          `${msg.auteur === "user" ? "Joueur" : "Personnage"} : ${msg.texte}`,
      )
      .join("\n");
...
// Demande à Yuna de générer un message personnalisé selon depuis
// combien de temps la personne n'est pas venue, et si elle a
// délaissé son Journal ou sa Galerie récemment — dans le ton de sa
// personnalité choisie, jamais culpabilisant.
// ============================================================
...
// ============================================================
// LÉGENDE IA POUR UNE PHOTO DE LA GALERIE
// Si l'image a une vraie photo (image.url en base64), Gemini la
// regarde réellement et écrit une légende dessus. Sinon (illustration
// décorative sans vraie photo), on lui donne juste le titre et le
